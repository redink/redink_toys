...
-export([new/0, is_empty/1, len/1, in/2, in_r/2, out/1, out_r/1, join/2,
         foldl/3, foldr/3, from_list/1, to_list/1, peek/1, peek_r/1, drop/1, member/2]).
...
-spec(drop/1      :: (?MODULE()) -> ?QUEUE:?QUEUE()).
-spec(member/2    :: (value(), ?MODULE()) -> boolean()).
...
member(_V, {0, _Q}) -> false;
member(V, {_L, Q}) -> ?QUEUE:member(V, Q).
